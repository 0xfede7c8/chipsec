...
The SIGCHECK, UNPACK and UTIL commands uses code from Igor Skochinsky.

	Github project: https://github.com/skochinsky/me-tools
	Author delivers his software with this licence that should be preserved:
...
The rest of the code by Federico Perez. 
Mail me if you need anything changed or fixed.
...
#---------------- START UNPACK FUNCTIONS AND CLASSES-------------------
...
def decomp_lzma(compdata):
		...
		path = basedir + '/../../chipsec_tools/linux/lzma'
		print "using decompressor at '%s'" % path 
		try:
				process = subprocess.Popen([path, "d"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def load_file(li, neflags, format):
		...
		return 1
#---------------- END UNPACK FUNCTIONS AND CLASES-------------------
...
class MECommand(BaseCommand):
	...
	cmds_help = [
		...
		"\tchipsec_util me unpack   <me_image.bin> : Dump and extract Intel ME fimrware images.",
	...
	def run(self):
		...
		if ("help" == op):
			...
			self.logger.log("[CHIPSEC]Intel ME dumper/extractor v0.3")
			if len(sys.argv) < 4:
				print "Usage: chipset_util me unpack MeImage.bin [-x] [offset]"
